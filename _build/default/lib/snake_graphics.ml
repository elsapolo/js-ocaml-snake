...
module Colors = struct
  ...
  let blue = Graphics.rgb 200 130 200
...
let draw_header ~game_state ~score1 ~score2=
  ...
  Graphics.draw_string (Printf.sprintf "Player1: %d." score1);
  Graphics.moveto (play_area_width - 75) (play_area_height + 10);
  Graphics.draw_string (Printf.sprintf "Player2: %d." score2)
;;
...
let render game =
  ...
  let snake1 = Game.snake1 game in
  let snake2 = Game.snake2 game in
  let score1 = Game.score1 game in
  let score2 = Game.score2 game in
  let (apple1, apple2) = Game.apples game in
  let game_state = Game.game_state game in
  draw_header ~game_state ~score1 ~score2;
  draw_play_area ();
  draw_apple apple1;
  draw_apple apple2;
  draw_snake (Snake.head snake1) (Snake.tail snake1) Colors.green;
  draw_snake (Snake.head snake2) (Snake.tail snake2) Colors.blue;
