...
let create ~length ~height ~width ~direction=
  match direction with
  | Direction.Left -> {
    direction = Left
  ; extensions_remaining = 0
  ; head = { Position.row = height - 1; col = width - length }
  ; tail = List.init (length - 1) ~f:(fun col -> { Position.row = height - 1; col = width - col - 1})
  }
  | _ -> { 
    direction = Right
  ; extensions_remaining = 0
...
let move_forward head tail direction =
  let head = Direction.next_position direction head in_channel_lengthl
...
let collides_with_self t =
  List.mem t.tail t.head ~equal:(Position.equal)
;;

let collides_with_other t s =
  List.mem (all_locations s) t.head ~equal:(Position.equal)
;;
...
let move_forward_and_grow ({ direction; extensions_remaining; head; tail } as t) =
  let newTail = match t.extensions_remaining with
  | 0 -> List.tl_exn (tail @ [head])
  | a -> (t.extensions_remaining <- a - 1;
  Fn.id) (tail @ [head])
  in Direction.next_position direction head, newTail
;;
...
let step t s =
  ...
  t.head <- head;
  t.tail <- tail;
  not (collides_with_self t) && not (collides_with_other t s)
;;
